<x-jet-action-section>
    <x-slot name="title">
        {{ trans('lang.log_title_se') }}
    </x-slot>

    <x-slot name="description">
        {{ trans('lang.log_tille_se_msg') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ trans('lang.log_se_text') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ trans('lang.this_device') }}</span>
                                    @else
                                        {{ trans('lang.last_active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ trans('lang.session_conf_pas_msg') }}
